...
// FUNGSI CROP BERSIH (Mencegah Background Hitam)
function getCleanCrop(ctx, img, canvas, offset) {
    ...
    if (imgRatio > canvasRatio) {
        dH = img.height; dW = img.height * canvasRatio;
        dX = (img.width - dW) / 2; dY = 0;
    } else {
        dW = img.width; dH = img.width / canvasRatio;
        dX = 0; dY = (img.height - dH) * (offset / 100);
    }
...
async function downloadManualCrop(i) {
    ...
    const link = document.createElement('a'); link.download = `FnD-Clean-${Date.now()}.png`;
